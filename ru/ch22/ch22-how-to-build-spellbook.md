...
После редактирования и сохранения тестового файла, снова используйте `dbt compile` для его компиляции. Внесите необходимые корректировки для устранения любых ошибок и обеспечения успешной компиляции.

На этом этапе важным шагом является ручное тестирование скомпилированного кода запроса на Dune для проверки его функциональности.

После успешной компиляции в каталоге `spellbook` будет создан подкаталог `target`. Перейдите в подкаталог `compiled/spellbook/models/spaceid/bnb`, где вы найдете файл `spaceid_bnb_registrations.sql`. Это SQL-определение представления за магической таблицей, которую мы строим. В том же каталоге есть подкаталог `spaceid_bnb_schema.yml`, содержащий автоматически сгенерированные тесты на основе определения схемы. Мы можем игнорировать эту часть.

Для ручного тестирования не рекомендуется копировать и запускать все содержимое файла запроса напрямую из-за большого объема данных. Вместо этого мы можем скопировать содержимое файла запроса в определение CTE, а затем выполнять запросы, возвращающие только небольшое количество данных, против этого CTE.
...
Полный запрос для ручного тестирования: https://dune.com/queries/2020131

Основная цель этого шага — убедиться, что скомпилированное SQL-заявление может выполняться правильно на Dune. Вы можете изменить окончательное утверждение выходного запроса для более подробного ручного тестирования.
## Отправка PR

Теперь мы обработали новую магическую таблицу и завершили локальное тестирование. Мы готовы добавить и закоммитить новые созданные и измененные файлы кода в репозиторий Github и отправить PR в Dune.

Сначала добавьте и закоммитьте новые созданные и измененные файлы в локальный git репозиторий. Затем отправьте локальную ветку в удаленный Github репозиторий:

```
# Проверьте и подтвердите новые созданные и измененные файлы и каталоги
git status

# Добавьте все новые созданные и измененные файлы для коммита
git add .

# Закоммитте в локальный git репозиторий
git commit -m 'Add spaceid view'

# Отправьте локальную ветку в удаленный Github репозиторий
git push -u origin add_bnb_spaceid
```

Часть `-u origin add_bnb_spaceid` в команде push требуется только для первого push. После первого push, если вы внесете дополнительные изменения в связанные файлы и закоммитите их в локальный git репозиторий, командой для отправки в Github будет:

```
git push
```

Далее, откройте веб-сайт Github и перейдите к репозиторию `spellbook` под вашим личным аккаунтом. Вы увидите уведомление, указывающее на то, что ветка `add_bnb_spaceid` отправила последние изменения, и для нее можно создать Pull Request (PR). Нажмите кнопку "Create PR", чтобы перейти на страницу создания PR.

На этой странице PR вам необходимо отредактировать и подтвердить следующее содержимое, убедившись, что вы выполнили соответствующие проверки и тесты, как это описано в документации. Редактор содержимого поддерживает синтаксис Markdown, и `[ ]` отобразит не отмеченный флажок, а `[x]` отобразит отмеченный флажок. Настройте эти опции одну за другой, чтобы подтвердить, что вы выполнили соответствующие шаги и действия.

Вам также необходимо предоставить краткое описание нового добавленного или измененного заклинания в PR, что облегчит рецензентам понимание изменений.

![](img/ch22_image_03.jpg)

После отправки PR вам нужно будет подождать проверки и обратной связи от соответствующих сторон. Также обращайте внимание на уведомления по электронной почте от Github. Некоторые уведомления можно игнорировать, например, те, что касаются `dbt slim ci (in beta) / dbt-test`. Основное внимание следует уделять обратной связи от рецензентов. Если необходимо внести изменения, переходите к внесению, тестированию, коммиту изменений в локальный git репозиторий и отправке в Github (изменения автоматически появятся под существующим PR без создания нового). Затем дождитесь следующего раунда проверки.

Если все изменения пройдут проверку без проблем, ваш PR будет объединен с основной веткой spellbook и развернут на производственном сайте Dune. После развертывания вы сможете искать и использовать созданную вами магическую таблицу в редакторе запросов. Поздравляем!
## Дополнительные примечания и особая благодарность

Примечание: на момент написания этого документа наш PR все еще находится на рассмотрении и не был одобрен. Дальнейшие изменения могут быть внесены на основе обратной связи. Номер PR для справки - 2725, и вы можете проверить детали на странице PR.

PR Github ссылка: [add BNB spaceid view](https://github.com/duneanalytics/spellbook/pull/2725)

Особую благодарность участнику сообщества @hosuke (Dune: https://dune.com/hosuke) за помощь в рассмотрении PR, предоставление обратной связи и предложений по улучшению, а также за помощь в изменении стратегии материализации магической таблицы.
## О нас

`Sixdegree` — это профессиональная команда по анализу данных на блокчейне. Наша миссия — предоставлять пользователям точные графики данных на блокчейне, аналитику и инсайты. Мы стремимся популяризировать анализ данных на блокчейне. Благодаря созданию сообщества, написанию учебных пособий и другим инициативам мы обучаем аналитиков данных на блокчейне, выпускаем ценный аналитический контент, продвигаем сообщество для построения слоя данных для блокчейна и воспитываем таланты для широкого будущего приложений данных блокчейна. Добро пожаловать на площадку обмена опытом сообщества!
