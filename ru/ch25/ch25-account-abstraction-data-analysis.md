...
## Анализ фабрики кошельков

Когда `контракт фабрики кошельков` создает смарт-кошелек, информация будет записана в таблицу `AccountDeployed`. Следовательно, мы можем просто использовать эту таблицу для анализа. Например, если вы хотите узнать количество созданных смарт-кошельков каждым фабрикой кошельков, вы можете использовать следующий SQL-запрос:
...
Стоит отметить, что эти бандлеры, платильщики и фабрики кошельков могут принадлежать одной и той же сущности. При анализе мы можем дополнительно связать эти адреса с метками сущностей, чтобы получить результаты, которые ближе к реальности. Метки для этих адресов можно найти здесь:

https://docs.google.com/spreadsheets/d/1QJEYDOr-AMD2bNAoupfjQJYJabFgdb2TRSyekdIfquM/edit#gid=0

Вышеприведенный анализ рассматривает только версию v0.6 контракта EntryPoint на Polygon. Если вы хотите проанализировать информацию в других сетях, вы можете проанализировать ее таким же образом. Конечно, вы также можете использовать агрегированные таблицы, которые мы предоставляем на Dune для анализа, что будет подробно представлено в следующей главе.