...
## Запрос информации о держателях нативного токена (ETH)

ETH является нативным токеном Ethereum, а не ERC20 токеном, и его данные о транзакциях не хранятся в таблице `evt_Transfer`, поэтому невозможно использовать их для расчета информации, такой как баланс ETH и держатели, так же, как это делается для ERC20 токенов. Смарт-контракт может переводить ETH одновременно с вызовом метода, поддерживающего функцию перевода, или даже при создании нового смарт-контракта (развертывании) или при самоуничтожении смарт-контракта. Кроме того, топливо для блокчейна Ethereum также оплачивается в ETH. Все эти переводы ETH записываются в таблицу `ethereum.traces`, в то время как таблица `ethereum.transactions` содержит только прямые транзакции.  Поэтому при расчете баланса ETH или подсчете количества держателей необходимо использовать таблицу `traces`. Реализация аналогична расчету баланса токена ERC20, за исключением источника данных. Вот панель мониторинга данных, созданная ранее для отслеживания баланса аккаунта ETH, и запрос демонстрирует реализацию.

Ссылка на панель мониторинга данных: [ETH Whales Tracking](https://dune.com/springzhang/eth-whales-top-1000-eth-holders)

## Рекомендованные источники

Панель данных "Советы и хитрости для запросов и визуализации в движке Dune V2" представляет собой сборник советов и хитростей для дальнейшего изучения. В дальнейшем будут продолжать добавлять и обновлять больше навыков, приглашаем к сбору.

Адрес панели: [Советы и хитрости для запросов и визуализации в движке Dune V2](https://dune.com/springzhang/tips-and-tricks-for-query-and-visualization-in-v2-engine)

## О нас

`Sixdegree` — профессиональная команда по анализу данных в блокчейне. Наша миссия — предоставлять пользователям точные графики, аналитику и инсайты по данным в блокчейне. Мы стремимся популяризировать анализ данных в блокчейне. Благодаря созданию сообщества и написанию учебных пособий, а также другим инициативам, мы обучаем аналитиков данных в блокчейне, выпускаем ценный аналитический контент, продвигаем сообщество для построения слоя данных блокчейна и воспитываем таланты для широкого будущего приложений данных блокчейна. Добро пожаловать на площадку обмена опытом!
